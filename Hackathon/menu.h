...
public:
	...
	void DrawOptions(Graphics* gfx, int n, int l, int w, int w_opt) {
		...
		for (int i = 1; i <= n; i++) {
			...
			gfx->DrawText(options[i - 1].c_str(), options[i - 1].size(), text_format, 0, posi + w_opt / 7, gfx->GetWindowWidth(), w_opt, black);
			posi += liber + w_opt;
		}
	}
	...
	void ChangeOption(int& button_num, int& w_opt, int& h, bool dif, bool mus, char* contrl) {
		...
		if (menu_num == 1) {
			...
			if (!dif) options[0] = L"Difficulty: Easy";
			else options[0] = L"Difficuly: Hard";

			if (mus) options[1] = L"Music: On";
			else options[1] = L"Music: Off";

			options[2] = L"Controls";
			options[3] = L"Exit";
			h = 400;
		}
		if (menu_num == 2) {
			...
			options[0] = L"Move left: " + left_key_str;
			options[1] = L"Move right: " + right_key_str;
			options[2] = L"Jump: " + jump_key_str;
			options[3] = L"Exit";
		}
			...
			if (input.GetKeyDown(VK_RETURN) && up_enter) {
				if (menu_num == 0)
					...
					if (pos == 1) {
						...
						if (on_music) {
							OutputDebugString(L"23131");
							options[1] = L"Music: Off";
							on_music = 0;
					...
					if (pos != 4) {
						get_nextkey = pos;
						if (pos == 1)
							options[pos - 1] = L"Move left: ";
						else if (pos == 2)
							options[pos - 1] = L"Move right: ";
						else
							options[pos - 1] = L"Jump: ";
						options[pos - 1] += L"...";
					}
					else {
						menu_num--;
